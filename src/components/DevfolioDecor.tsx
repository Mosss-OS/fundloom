...
export function SmileMoon({ className = "", color = "#F4C26B" }: { className?: string; color?: string }) {
  return (
    <svg viewBox="0 0 80 50" className={className} aria-hidden>
      <path
        d="M2 6 C 2 30, 30 48, 40 48 C 50 48, 78 30, 78 6 Z"
        fill={color}
      />
      <circle cx="28" cy="22" r="2.4" fill="#1a1a1a" />
      <circle cx="52" cy="22" r="2.4" fill="#1a1a1a" />
      <path d="M30 32 Q 40 40 50 32" stroke="#1a1a1a" strokeWidth="2.2" fill="none" strokeLinecap="round" />
    </svg>
  );
}

export function Sunburst({ className = "", color = "#E8E4D5" }: { className?: string; color?: string }) {
  // 12-point soft star
...
export function Blob({ className = "", color = "#9CA8FF" }: { className?: string; color?: string }) {
  return (
